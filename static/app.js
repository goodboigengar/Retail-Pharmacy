(() => {
  ...
  // Map of drug_id -> {id, generic_name, brand_names, otc_or_rx}
  const selected = new Map();

  function otcBadge(otcOrRx) {
    const v = (otcOrRx || "").toLowerCase();
    let cls = "rx", label = "Rx";
    if (v === "otc") { cls = "otc"; label = "OTC"; }
    else if (v.includes("/")) { cls = "mixed"; label = otcOrRx; }
    else if (v.includes("controlled")) { cls = "rx"; label = "Rx"; }
    let extra = "";
  ...
  function renderSelected() {
    ...
    for (const d of selected.values()) {
      const li = document.createElement("li");
      const brandStr = d.brand_names && d.brand_names.length ? ` (${d.brand_names.join(", ")})` : "";
      li.innerHTML = `<span>${d.generic_name}${brandStr} ${otcBadge(d.otc_or_rx)}</span>
        <button class="remove-btn" data-id="${d.id}" title="Remove">✕</button>`;
      li.querySelector(".remove-btn").addEventListener("click", () => {
        selected.delete(d.id);
        renderSelected();
        // Uncheck in results list if visible
  ...
  function renderDrugConsultation(d) {
    ...
    return `
      <div class="drug-consult">
        <h3>${d.generic_name}${brandStr} ${otcBadge(d.otc_or_rx)}</h3>
    ...
    fetch("/api/consultation", {
      ...
      .then(async (r) => {
        if (!r.ok) {
          ...
          throw new Error(err.error || "Failed to generate consultation.");
        }
        return r.json();
      })
      .then((data) => {
